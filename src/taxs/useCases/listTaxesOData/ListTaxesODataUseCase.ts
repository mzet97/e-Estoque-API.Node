...
import ITaxesRepository, { TaxFilters } from '../../repositories/ITaxesRepository'
import { ODataQuery } from '@shared/services/ODataParser'
import { Request } from 'express'

export interface ListTaxesODataFilters {
...
@injectable()
export default class ListTaxesODataUseCase extends BaseODataUseCase<ListTaxesODataFilters, IPaginationResult<any>> {
  constructor(
    ...
    @inject('TaxesRepository')
    private taxesRepository: ITaxesRepository,
  ) {
    super(oDataParser, oDataCache)
  }

  async execute(filters: ListTaxesODataFilters): Promise<IResult<IPaginationResult<any>>> {
    try {
      console.log('ListTaxesODataUseCase.execute:', filters)

      const { oDataQuery, userId, cacheEnabled = true, includeCount = false } = filters

      // If no OData query, use standard listing
      if (!oDataQuery) {
        const result = await this.taxesRepository.findWithFilters({
          page: 1,
          pageSize: 15,
          orderBy: 'name',
          orderDirection: 'ASC'
        ...
        return {
          success: true,
          data: result,
          message: 'Impostos listados com sucesso'
        }
      }
      ...
      const result = await this.handleWithCache(
        ...
        userId,
        async () => {
          // Define allowed fields for Tax entity
          const entityFields = [
            'id', 'name', 'percentage', 'description', 'isActive',
          ...
          const repositoryResult = await this.taxesRepository.findWithFilters(repositoryFilters)

          // Handle $count
          let total = repositoryResult.total
          if (oDataQuery.count) {
            total = await this.taxesRepository.count()
          }
      ...
      return {
        success: true,
        data: result,
        message: 'Impostos listados com sucesso com OData',
        meta: {
          oDataQuery,
      ...
      console.error('Error in ListTaxesODataUseCase:', error)
      return {
        success: false,
        data: null,
        message: 'Erro interno ao listar impostos com OData'
      }
    }
  }
}
